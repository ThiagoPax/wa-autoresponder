...
class TennisNotificationListener : NotificationListenerService() {
    ...
    private fun processNotification(sbn: StatusBarNotification) {
        ...
        // Log body length for debug
        log("Tamanho do texto: ${body.length} caracteres")
        
        if (body.isBlank()) {
            log("Corpo da mensagem vazio")
            return
        }
        ...
        if (!result.canRespond) {
            log("${result.reason}")
            // Log first 200 chars of body for debug
            log("Preview: ${body.take(200)}...")
            return
        }
    ...
    private fun extractBody(extras: Bundle): String {
        ...
        // Try summary text
        extras.getCharSequence("android.summaryText")?.toString()?.let {
            if (it.isNotBlank()) results.add(it)
        }
        ...
        // Return the longest text found (most complete)
        return results.maxByOrNull { it.length } ?: ""
    }
